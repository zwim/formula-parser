...
function Parser:_parse(str)
    if not str or str:len() == 0 then return Node:new{} end

    -- do this first, heuristically, we have always numbers
    local value = tonumber(str)
    if value then return Node:new{val = value} end
...
function Parser.basicOperator(str, operator, right)
    ...
    if right == 1 then
        ...
        until ((opPos > str_len or str:sub(opPos, opPos + opLen - 1) == operator) and
            (charRight ~= "=" -- do this because "=" has higher priority than e.g. ">="
            and charLeft ~= "<" and charLeft ~= ">" and charLeft ~= "=" and
                charLeft ~= "!"))
    else
        opPos = str_len + 1
        repeat
            ...
            charRight = str:sub(opPos + opLen, opPos + opLen)
            charLeft = str:sub(opPos - 1, opPos - 1)
        until ((opPos <= 0 or str:sub(opPos, opPos + opLen - 1) == operator) and
            isLeftNum == 0 and charRight ~= "=" -- do this because "=" has higher priority than e.g. ">="
...
function Parser.ternaryOperator(str, operator)
    ...
    if opPos <= str_len then
        ...
        if opPos <= str_len then return opPos1, opPos end
    end
end
